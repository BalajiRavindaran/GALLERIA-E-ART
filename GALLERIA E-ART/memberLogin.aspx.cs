        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            if (validations())
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SELECT * FROM member_profile_table WHERE member_id='" + TextBox1.Text.Trim() + "' AND password='" + TextBox2.Text.Trim() + "';", con);
                    ...
                    if (dataReader.HasRows)
                    {
                        while (dataReader.Read())
                        {
                            ...
                            Session["role"] = dataReader.GetValue(3).ToString();
